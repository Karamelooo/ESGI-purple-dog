                ...
                <table className="min-w-full text-sm">
                    ...
                    <tbody className="divide-y divide-gray-100">
                        {transactions.length === 0 && (
                            <tr><td colSpan={6} className="px-6 py-8 text-center text-gray-500">Aucune transaction pour le moment.</td></tr>
                        )}
                        {transactions.map((t) => (
                            <tr key={t.id} className="hover:bg-gray-50">
                                ...
                                <td className="px-6 py-4">
                                    <span className="px-2 py-1 rounded-full text-xs font-bold bg-gray-100 text-gray-700">{t.type}</span>
                                </td>
                                <td className="px-6 py-4 text-right font-medium">{t.amount} €</td>
                                <td className="px-6 py-4 text-right font-bold text-green-600">+{t.commissionAmount} €</td>
                                <td className="px-6 py-4 text-right text-gray-400">{new Date(t.createdAt).toLocaleDateString()}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
